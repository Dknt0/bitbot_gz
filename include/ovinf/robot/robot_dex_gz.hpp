#ifndef ROBOT_DEX_GZ_HPP
#define ROBOT_DEX_GZ_HPP
...
class RobotDexGz : public RobotBase<float> {
  using VectorT = Eigen::Matrix<float, Eigen::Dynamic, 1>;
  friend class ObserverDexGz;
  friend class ExecutorDexGz;

 public:
  using Ptr = std::shared_ptr<RobotDexGz>;

 private:
  class ObserverDexGz : public ObserverBase {
   public:
    ObserverDexGz() = delete;
    ObserverDexGz(RobotBase<float>* robot, const YAML::Node& config)
    ...
    virtual VectorT const& Depth() const override {
      auto robot_gz = dynamic_cast<RobotDexGz*>(robot_);
  ...
  class ExecutorDexGz : public ExecutorBase {
   public:
    ExecutorDexGz() = delete;
    ExecutorDexGz(RobotBase<float>* robot, const YAML::Node& config)
 ...
 public:
  RobotDexGz() = delete;
  RobotDexGz(const YAML::Node& config) : RobotBase(config) {
    motors_.resize(motor_size_);
    this->observer_ = std::make_shared<ObserverDexGz>((RobotBase<float>*)this,
                                                      config["observer"]);
    this->executor_ = std::make_shared<ExecutorDexGz>((RobotBase<float>*)this,
                                                      config["executor"]);
  }
...
void RobotDexGz::GetDevice(const KernelBus& bus) {
  for (size_t i = 0; i < motor_size_; ++i) {
...
void RobotDexGz::ObserverDexGz::CreateLog(YAML::Node const& config) {
...
void RobotDexGz::ObserverDexGz::WriteLog() {
